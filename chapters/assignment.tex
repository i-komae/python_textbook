...
から入手できる。手元に配布物がそろっていない場合は、まずリポジトリを取得し、リポジトリ直下にある \code{slow\_event\_analysis.py} と \code{events\_data.zip} の配置を確認する。\code{events\_data.zip} を展開すると \code{events\_data/} が得られ、これを入力ディレクトリとして使う。

配布物は意図的に最小限にしてある。したがって、何を入力とし、どのファイルが正解判定の対象で、どこからどこまでを自分で実装するのかを最初に整理してから着手した方がよい。

\subsection{まず配布コードを動かす}
...
全期間の処理時間は、データ量、ストレージ、計算機によって変わる。初回は数日分だけを切り出し、入力形式と出力を確認してから対象期間を広げる。

重要なのは、最初から完成版を書こうとしないことである。まず配布コードが本当に遅いことを自分の環境で確認し、どの出力が得られるかを見てから変更を始める方が、修正の意味が分かりやすい。

\subsection{gzip ファイルの中身を見る}
