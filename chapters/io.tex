...
Python でテキストファイルを読むときの基本は \code{open} である。さらに \code{with} を組み合わせると、読み終えた後に自動でファイルが閉じられる。テキストファイルを 1 行ずつ読む基本形は次のようになる。
...
ファイルパスを単なる文字列として扱うこともできるが、Python では次のように \code{pathlib.Path} を使うと見通しが良くなる。
...
ログや測定結果が複数ファイルに分かれている場合は、まず対象ファイルの一覧を作る必要がある。Python では \code{glob.glob} がよく使われる。基本的な探索方法は次の通りである。
...
CSV は一見単純に見えるが、引用符やカンマを含む列があると \code{line.split(",")} では壊れやすい。列名付きの表を扱うなら、標準ライブラリの \code{csv} を使う方が安全である。
...
\code{DictReader} は 1 行目を列名として読み、その後の各行を辞書として返す。したがって \code{row["mag"]} のように列名で値を取り出せる。\code{newline=""} を付けるのは、CSV モジュールが改行を正しく扱いやすくするためである。
...
結果や設定を構造化して保存したいときは、JSON が便利である。テキストとして読めて、Python では標準ライブラリ \code{json} ですぐ扱える。基本的な保存と読込みの記述は次のように行う。
...
Python オブジェクトをそのまま保存したいときは、\code{pickle} もよく使われる。拡張子は慣習的に \code{.pkl} を使うことが多い。辞書、リスト、NumPy 配列、学習済みモデル、中間集計結果などを素早く退避するには便利である。処理の記述例を下に示す。
...
日付や時刻を含むデータでは、文字列のまま扱うより、比較や計算がしやすい形へ変換した方がよい。標準ライブラリの \code{datetime} は、そのための基本的な道具となる。実際のコードは次のようになる。
