...
Astropy\index{astropy@Astropy|textbf} は、天文学や宇宙物理の解析で頻出する「単位付き物理量」「時刻系」「座標系」「FITS」「天文向け表」を安全に扱うためのライブラリである。NumPy や SciPy だけでも数値計算はできるが、単位や時刻系を自前で文字列管理すると、解釈違いや換算ミスが入りやすい。本章では、Astropy の考え方、最小限のコード例、落とし穴、解析へのつながりを整理する。
...
\]
である。コードと単位換算を一緒に見ておくと、桁や接頭辞の取り違えに気付きやすい。
...
\]
で定義される。Astropy は、このような時刻表現どうしの対応を自前で管理せずに済むようにしてくれる。
...
\]
である。重要なのは、コード上では単に引き算しているだけでも、背後で時刻系と単位が保たれている点である。
...
\code{Table.read()} の第 1 引数は入力パス、\code{format="ascii.ecsv"} は ECSV として解釈する指定である。例えば 3 行で \code{time} と \code{signal} の 2 列を持つ表なら、\code{['time', 'signal']} と \code{3} が表示される。ECSV は単位やメタデータを持たせやすいので、テキスト形式のまま情報を落としにくい。共同研究や中間生成物の受け渡しでは便利である。保存するときは \code{table.write("events.ecsv", format="ascii.ecsv", overwrite=True)} と書ける。\code{overwrite=True} は同名ファイルの置換を許可する指定である。
...
一方で、巨大な表の集計や複雑な join を高速に回すなら、pandas や Polars の方が便利である。実務では、まず Astropy で単位と時刻を正しく解釈し、その後に必要な部分だけを pandas や Polars へ渡して集計し、最後に Matplotlib や SciPy へつなぐことが多い。

ここまでの内容を公式資料で引き直すなら、まず Getting Started~\cite{astropy_getting_started} と User Guide~\cite{astropy_user_guide} を入口にするとよい。その上で、単位は Units and Quantities~\cite{astropy_units}、時刻は Time and Dates~\cite{astropy_time}、座標は Coordinates~\cite{astropy_coordinates}、表は Tables~\cite{astropy_table}、FITS は FITS File Handling~\cite{astropy_fits} が対応している。本章では、それらの中でも観測データ解析で頻繁に使う順に絞って説明した。

\section{章末問題}

\begin{enumerate}
\item \textbf{単位を保ったまま時間窓を比較する}。
...
\item \textbf{秒境界をまたぐ時刻差を求める}。
\code{2026-04-12T11:59:59.999999900} と \code{2026-04-12T12:00:00.000000120} を UTC の \code{Time} として作り、差を ns で表示せよ。結果は約 220 ns になり、250 ns 以内である。\code{format="isot"}、\code{scale="utc"}、\code{to\_value("ns")} の役割を説明すること。

\item \textbf{UTC と TAI を区別して同一時刻を表す}。
...
\item \textbf{単位付き表を ECSV と FITS へ保存する}。
\code{time}、\code{signal}、\code{detector} を持つ \code{QTable} を作り、\code{time} に ns、\code{signal} に mV を付ける。ECSV へ保存して読み戻し、各列の単位と行数を表示せよ。さらに同じ信号値を FITS 画像として保存し、\code{fits.open()} で形状とヘッダを確認せよ。\code{overwrite=True} を付ける場合の挙動も説明すること。

\item \textbf{座標系を明示して変換する}。
...
ここまでに、データの読込み、整形、計算、可視化、物理量の表現を一通り扱った。同じ解析を別のデータへ安全に適用するには、処理を関数とモジュールへ分け、テスト可能な公開インタフェースを定める必要がある。次章では、そのためのパッケージ構成と依存関係の管理を扱う。
