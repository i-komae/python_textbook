...
Matplotlib\index{matplotlib@Matplotlib|textbf} は、Python で図を描くための代表的なライブラリである。本章では、データの性質に応じた図の選択と、オブジェクト指向 API による構成要素の制御を扱う。前半では基本的な作図と図種の選択、後半では Figure、Axes、Axis、Artist の関係、複数パネル、スタイル、保存方法を説明する。
...
図~\ref{fig:scatter-bad} からは点が存在する範囲は分かるが、各領域に点が何個あるかは読み取れない。密度を比較する目的には、2 次元ヒストグラムの方が適している。
...
図~\ref{fig:hist2d-lognorm}では、単一のピークの周囲に広がる緩やかな相関層まで判別できる。この例は、数桁に及ぶ密度差を同一図で比較する際に LogNorm が有効であることを示している。
...
\subsection{設定ファイル (.mplstyle) による描画設定の共通化}\index{Matplotlib!.mplstyle@\code{.mplstyle}|textbf}
...
作りたいグラフのイメージが先にあるなら、まず \code{Plot types} で図の種類を当て、次に \code{Gallery} で近い完成例を探し、最後に \code{Cheatsheets} で引数名や書式を確認すると迷いにくい。\code{Quick start} は、\code{Figure} と \code{Axes} の関係を短時間で思い出したいときに向いている。初学者が最初から完全なコードを自力で組み立てるより、近い例を見つけて必要な部分だけ差し替える方が現実的である。

\section{地震カタログと発生間隔の可視化}
...
第~\ref{chap:numpy}~章で作った \(\Delta t\) 配列を用い、発生間隔ヒストグラムを描け。少なくとも縦軸は対数表示にし、必要なら横軸も対数にして、どの表示が見やすいかを比較せよ。図中にはビンの意味が分かる軸ラベルと単位を明記せよ。
