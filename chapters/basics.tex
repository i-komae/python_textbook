...
浮動小数点数の多くは 2 進数で有限桁に表現できないため、表示値と内部値が数学的に厳密には一致しない場合がある。この丸め誤差は、後の章で扱う統計量とフィット結果の比較にも影響する。
...
文字列には、分割、検索、置換、前後の空白除去などの操作を適用できる。各メソッドの入力と返り値を対話モードで確認する。
...
f 文字列\index{もじれつ@文字列!fもじれつ@f文字列|textbf}を用いると、変数の値と書式指定を文字列へ埋め込める。解析コードでは、ログ、図の保存名、結果ファイル名、単位付きの数値表示などに利用する。
...
この動作は関数呼び出しの間で状態を共有するため、意図しない結果になりやすい。変更可能な既定値を避けることで防止できる。
