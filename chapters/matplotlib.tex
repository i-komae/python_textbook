...
線図には \code{plot()}\index{Matplotlib!plot@\code{plot()}|textbf}、散布図には \code{scatter()}\index{Matplotlib!scatter@\code{scatter()}|textbf}、鉛直および水平の参照線には \code{axvline()}\index{Matplotlib!axvline@\code{axvline()}} と \code{axhline()}\index{Matplotlib!axhline@\code{axhline()}}、誤差付きの点には \code{errorbar()}\index{Matplotlib!errorbar@\code{errorbar()}|textbf} を用いる。次の例は、これらの関数と出力図の対応を示す。
...
格子上で定義された 2 次元配列は \code{imshow()}\index{Matplotlib!imshow@\code{imshow()}|textbf} で色に対応させる。これに対して、点群を格子ごとに数え上げる場合は \code{hist2d()}\index{Matplotlib!hist2d@\code{hist2d()}|textbf} を用いる。いずれの場合も、色が表す量と尺度をカラーバーに記す。
...
国境、海岸線、地理座標系の変換を必要とする地図には Cartopy を用いる。球面座標上の点分布だけを表示する場合は、Matplotlib の標準投影を利用できる。入力座標の単位、経度の範囲、周期境界における処理を作図前に確認する。
...
物理イベントの頻度が 1、10、100、1000 のように数桁にわたる場合には、線形カラースケールだけでは低密度領域の差を表しにくい。\code{LogNorm} は、正のカウントを対数尺度で色へ割り当てる。\index{Matplotlib!LogNorm@\code{LogNorm}|textbf}
...
図~\ref{fig:hist2d-lognorm}では、単一のピークの周囲に広がる低密度の相関層も判別できる。線形尺度では近い色に対応していた低密度領域が、対数尺度では異なる色に割り当てられるためである。

\subsection{\code{subplots()} による複数パネルの配置}

関連する分布を同じ尺度で比較する場合は、1 枚のキャンバス（\code{Figure}\index{Matplotlib!Figure@\code{Figure}|textbf}）の中に複数のパネル（\code{Axes}\index{Matplotlib!Axes@\code{Axes}|textbf}）を配置する。
...
次の例では、同じ物理量を条件 A と B で測定した状況を正規乱数で模擬する。両パネルに同じビン数と共有した縦軸を用い、分布の位置と幅を比較する。
...
\begin{itemize}
    ...
    \item \textbf{ラスタ形式 (\code{.png}, \code{.jpg})}：図をピクセルの集合として保存するため、拡大率によっては輪郭に画素が見える。ベクタ形式では各点が個別要素となるので、密集した散布図や大きな格子画像ではファイルサイズと描画負荷が増える。この場合は、\code{savefig("plot.png", dpi=150)} のように必要な解像度を指定して PNG で保存する。
\end{itemize}
...
図~\ref{fig:anatomy-example} に、Figure、Axes、Axis、Artist の位置関係を示す。Figure は一つ以上の Axes を持ち、各 Axes は x 軸と y 軸を表す Axis、および線、文字、凡例などの Artist を持つ。
...
Matplotlib では、点の形\index{marker@marker}、線種\index{linestyle@linestyle}、色をそれぞれ独立に指定できる。対応する主な引数は次の 4 つである。
...
単位を数式の中へ入れる場合は、上の例のように \code{\\mathrm} を使って立体で表す。図中の記号、字体、単位は、本文での定義と一致させる。

MathText は、Matplotlib 内部で数式の一部を組版する機能であり、LaTeX の処理系そのものではない。分数、上付き・下付き、ギリシャ文字、\code{\\mathrm} などを扱える一方、任意の LaTeX マクロやパッケージ依存の記法には対応しない。本文と同じ LaTeX 処理系による組版が必要な場合は、外部依存関係を記録したうえで \code{text.usetex=True} を検討する。
