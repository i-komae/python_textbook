...
\code{select()} で列が減り、\code{filter()} で条件に合う行だけが残る。出力の例を示す。表の罫線や型の表示は一部簡略化している。
...
一つの \code{with\_columns()} 内の式は、基本的に呼び出し前の列を参照する。同じ呼び出しで新しく作る列を別の式から使う場合は、\code{with\_columns()} を二段に分ける。
...
集計後は、検出器ごとに 1 行ずつの表へまとまる。出力の例を示す。表の罫線や型の表示は一部簡略化している。
...
\code{pl.len()} は群の行数、\code{pl.col("signal").count()} はその列の非 null 値の個数を数える。標準偏差の既定は \code{ddof=1} である。群の平均を元の各行へ付けるには、\code{over()} を使う。
\begin{lstlisting}[language=Python]
with_group_mean = derived.with_columns(
    pl.col("signal").mean().over("detector").alias("detector_mean")
)
\end{lstlisting}
\code{group\_by().agg()} が群ごとに行をまとめるのに対し、この \code{over()} は元の行数を保つ。
...
\code{when().then().otherwise()} では、Python の \code{if} のように選ばれた分岐だけが評価されるとは限らない。各分岐の式は、それぞれ単独でも有効でなければならない。条件外の行で不正な型変換が起きる場合も、分岐を書くだけでは回避できない。
...
\code{scan\_parquet()} の引数 \code{"events/*.parquet"} は、一致する複数の Parquet ファイルを入力にする。返り値は \code{LazyFrame} であり、この時点ではデータを実体化しない。\code{collect()} を呼ぶとクエリが実行され、返り値として DataFrame が得られる。実行時には、\code{select()} で使う列だけを読む列射影や、\code{filter()} を早い段階へ移す最適化が適用されることがある。最適化内容を確認したい場合は、実行前に \code{print(lf.explain())} として計画を表示できる。
...
遅延実行は、必要列の選択や条件による絞り込みを実行計画へ反映する仕組みであり、メモリ使用量を無制限に小さくするものではない。\code{collect()} で最終的に得る表の大きさと、並べ替え・結合の中間データにも注意する。
...
\section{章末課題：検出器別集計と遅延実行}

共通入力には前章の \code{events.csv} を用いよ。ファイルがない場合は、第~\ref{chap:pandas}~章の章末課題に示した 6 行の CSV を作成せよ。
