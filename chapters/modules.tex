...
別の仮想環境を 2 つ作り、一方では \code{uv pip install .}、もう一方では \code{uv pip install -e .} を実行せよ。インストール後に関数の返り値を変更し、どちらの環境で変更が直ちに反映されるかを確認せよ。\code{-e} はソースディレクトリを参照可能な形で登録する指定であり、依存関係の解決やビルド処理をすべて省略する指定ではない。
