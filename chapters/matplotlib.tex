...
点が重なって密度を読み取れない場合には、領域ごとの点数を色で表す 2 次元ヒストグラム（\code{hist2d}）が有効である。
...
図~\ref{fig:hist2d-linear} では中心部の密度を読み取れるが、最大値が約 2 万カウントに達するため、10 カウントや 100 カウント程度の領域には近い色が割り当てられる。この線形カラースケールでは、周辺部の密度差や裾の広がりを識別しにくい。

\subsubsection{\code{LogNorm} による広い密度範囲の表示}

物理イベントの頻度が 1、10、100、1000 のように数桁にわたる場合には、線形カラースケールだけでは低密度領域の差を表しにくい。カウントを対数に変換して色へ割り当てる \code{LogNorm} は、このような分布に適している。\index{Matplotlib!LogNorm@\code{LogNorm}|textbf}
...
レポートや論文では、関連する分布を左右や上下に並べて比較することが多い。この場合、1 枚のキャンバス（\code{Figure}\index{Matplotlib!Figure@\code{Figure}|textbf}）の中に複数のパネル（\code{Axes}\index{Matplotlib!Axes@\code{Axes}|textbf}）を配置する機能を利用する。
...
\begin{lstlisting}[language=Python]
fig, axes = plt.subplots(1, 2, figsize=(10, 4)) # 横 10 インチ、縦 4 インチで 1 行 2 列のパネルを作成
...
\begin{itemize}
    \item \textbf{ベクタ形式 (\code{.pdf}, \code{.svg}, \code{.eps})}：線や文字を図形要素として保存するため、拡大しても輪郭がぼやけない。線図や要素数の少ない散布図を論文やレポートへ掲載する場合に適している。\code{savefig("plot.pdf")} のように保存する。
    \item \textbf{ラスタ形式 (\code{.png}, \code{.jpg})}：図をピクセルの集合として保存するため、拡大率によっては輪郭に画素が見える。ベクタ形式では各点が個別要素となるので、密集した散布図や大きな格子画像ではファイルサイズと描画負荷が増える。この場合は、\code{savefig("plot.png", dpi=150)} のように解像度を指定した PNG が適している。
\end{itemize}
...
保存には \code{fig.savefig(...)}、凡例には \code{ax.legend()}、対数軸には \code{ax.set\_xscale("log")} を用いるのは、それぞれの操作対象が Figure または Axes だからである。各メソッドの呼出し元が \code{fig} と \code{ax} のどちらかを追うと、処理対象を判別しやすい。
...
図~\ref{fig:style-reference-example} は、よく使うマーカー、線種、既定の色循環を並べたものである。各指定と描画結果の対応を知っておくと、コード中の短い記号から線や点の形を判別できる。
...
解析テーマごとに作図スクリプトが増えると、各 Python ファイルの先頭へ同じ設定辞書をコピーする方法は保守しにくい。Matplotlib では、これらの設定をプレーンテキストへ分離し、まとめて読み込める。
