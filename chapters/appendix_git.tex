...
\item 組織内（\code{internal}）: 組織内の利用者だけが閲覧できる。主に GitHub Enterprise で用いられる
\end{itemize}
...
\code{uv} を使うプロジェクトでは、通常 \code{.venv/} はコミットしない。一方で、\code{pyproject.toml}、\code{uv.lock}、\code{.gitignore}、そしてプロジェクト単位でバージョンを固定したいなら \code{.python-version} はコミット候補になる。
...
\code{git clone} は、履歴も含めて手元へ持ってくる。\code{git remote -v} は、どの URL が接続先として設定されているかを確認する。本書では、GitHub から取得するときも push するときも SSH URL を使う方針を勧める。公開鍵の準備は付録~\ref{app:ssh} を参照してほしい。
...
\code{git clone}、\code{pull}、\code{push} を通して運用をそろえるなら、本書では SSH を標準にする方が自然だと考える。特に、公開リポジトリを読む段階では HTTPS でも動くが、後で push や private repository の取得が必要になると認証方式が増えて煩雑になりやすい。そのため、最初から SSH へそろえておく方が混乱が少ない。
...
GitHub では、HTTPS 経由の Git 操作で通常のアカウントパスワードは使えない。現在は、SSH 鍵を登録して使うか、個人用アクセストークン (PAT) を使うのが基本である。本書では SSH 鍵を使う方法を標準とし、HTTPS + PAT は事情がある場合の代替手段として扱う。

\subsubsection{SSH 鍵による認証}

SSH を使う場合は、付録~\ref{app:ssh} を参照して公開鍵を用意し、それを GitHub の \code{SSH and GPG keys} へ登録する。GitHub CLI \code{gh auth login --git-protocol ssh --web} を使うと、その作業を対話的に進めやすい。
