...
\code{chmod u+x} はファイル所有者に実行権限を追加し、\code{./run.sh} は現在のディレクトリにあるファイルを実行する。最後の行がスクリプトの標準出力である。
...
\begin{itemize}
    ...
    \item \code{\$\#}: 渡された引数の総数
    \item \code{\$@}: 渡されたすべての引数。個々の引数を保ったまま展開する場合は \code{"\$@"} と書く
\end{itemize}
...
if [ -f "$FILE" ]; then
    ...
    echo "エラー: $FILE はディレクトリです。ファイルを指定してください。" >&2
    exit 1
else
    echo "エラー: $FILE が存在しません。" >&2
    exit 1
fi
\end{lstlisting}
...
\code{if} は直後のコマンドの終了ステータスが 0 なら条件成立とする。\code{[ ... ]} も条件を調べるコマンドであり、括弧の内側の空白が必要である。\code{command1 \&\& command2} は前段が成功した場合だけ、\code{command1 || command2} は失敗した場合だけ後段を実行する。エラーを表示するだけでは処理は止まらないため、必要な箇所で \code{exit 1} を使う。詳細は Bash の公式資料~\cite{bash_manual} を参照する。

\subsection{複数ファイルの入出力パス生成と一括処理}
\index{ばっちしょり@バッチ処理|textbf}

特定のディレクトリにある複数のファイルを順次処理する場合にも、\code{for} ループを利用できる。次の例では、入力ファイル名（\code{data/input/run01.csv} など）から、対応する出力ファイル名（\code{data/output/run01\_result.png} など）を生成する。
...
for filepath in "$INPUT_DIR"/*.csv; do
    # 一致がない場合や、名前が .csv で終わるディレクトリは処理しない
    if [ ! -f "$filepath" ]; then
        continue
    fi
...
\section{\code{sed} による条件別設定ファイルの生成}
\index{sed@\code{sed}|textbf}

外部プログラムには、\code{--input XXX} のようなコマンドライン引数ではなく、\code{config.ini} などの設定ファイルから実行条件を読み込むものもある。複数のシード値や閾値を比較する場合は、条件ごとの設定ファイルをテンプレートから生成すると、各出力に対応する実行条件を保存できる。
...
for thresh in "${thresholds[@]}"; do
    ...
    run_config="config_run_${thresh}.ini"
