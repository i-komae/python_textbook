...
Jupyter の \code{\%timeit}\index{timeit@\code{\%timeit}|textbf} と \code{\%\%timeit} は、短い式またはセルを複数回実行し、所要時間の統計量を表示する。短い処理の比較に用いる。
...
たとえば $N = 10^6$ なら、$\log_2 N$ はおよそ 20、$N \log_2 N$ はおよそ $2 \times 10^7$、$N^2$ は $10^{12}$ である。入力サイズが同じでも、処理回数の見積もりには約 5 万倍の差がある。小さなテストデータでは判別しにくい差も、入力が大きくなると処理時間を支配し得る。
