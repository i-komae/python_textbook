...
Polars\index{polars@Polars|textbf} は、列指向の DataFrame 処理を高速に行うためのライブラリである。pandas と同じく表形式データを扱うが、index を持たず、式を組み合わせて列変換を書く点が異なる。さらに、\code{LazyFrame}\index{lazyframe@LazyFrame|textbf} では、実行前に処理全体を最適化できる。本章では、ライブラリが利用者へ提供する操作の体系（API）を、直ちに処理する eager API と、処理を蓄えてから実行する lazy API に分けて説明し、集計、結合、入出力を扱う。
...
表示されるのは、概ね \code{[(col("signal")) / (col("signal").mean())]} のような式であり、4 行分の数値ではない。この段階では、まだ計算結果ではなく「どう計算するか」を表している。実際の値を得るには、\code{df.select(expr.alias("signal\_ratio"))} のように DataFrame の文脈へ式を渡す。Polars の公式ドキュメント~\cite{polars_expressions} でも、expression は列処理を構成する基本概念として説明されている。\code{select()}、\code{with\_columns()}、\code{filter()}、\code{group\_by()} の多くは、式を受け取る API で統一されている。
