...
Parquet は保存形式の名前であり、\code{pyarrow} は pandas からその形式を読み書きする代表的なライブラリである。したがって、「Parquet を使う」と「\code{pyarrow} を入れる」は同じ話ではない。\code{read\_parquet()} や \code{to\_parquet()} を使う環境に対応ライブラリが入っていなければ、Parquet の読書きは実行できない。
