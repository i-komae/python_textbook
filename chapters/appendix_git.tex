...
ローカルとリモートの双方で同じ名前の README や LICENSE を作成している場合、\code{git pull} によって競合が生じることがある。競合箇所を編集し、\code{git add} と \code{git commit} で統合結果を記録してから、\code{git push -u origin main} を実行する。
