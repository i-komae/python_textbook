...
distance = (speed * time_window).to(u.m)

print(f"{distance.to_value(u.m):.7f} m")
print(f"{distance.to_value(u.km):.10f} km")
\end{lstlisting}
...
UTC（協定世界時）、TAI（国際原子時）、TT（地球時）は時刻系であり、同じ瞬間に付ける数値の基準が異なる。一方、JD（ユリウス日）、MJD（修正ユリウス日）、Unix time は時刻の表現形式である。\(\mathrm{MJD}=\mathrm{JD}-2400000.5\) であり、どの時刻系で表した値かを併記する。時刻系の指定と、日時を文字列や日数で表示する形式の指定は別の操作である~\cite{astropy_time}。
...
print(velocity)
print(f"{velocity.to_value(u.m / u.s):.1f} m / s")
\end{lstlisting}
...
Astropy の \code{Time} は内部で時刻を二つの浮動小数点数に分けて保持する。\code{t.unix} などを単一の浮動小数点数として取り出すと、元の時間分解能が失われる場合がある。ns 程度の差を調べるときは \code{Time} のまま減算してから単位変換する。また、表示桁数は計算精度とは別であり、\code{t.precision = 9} などで変更できる。
...
\code{ra} は赤経（right ascension）、\code{dec} は赤緯（declination）を表し、\code{icrs} は国際天球基準座標系を指定する。\code{ra} と \code{dec} には単位を付ける。これにより、入力角度が度またはラジアンのどちらで与えられたかが \code{SkyCoord} に保持される。
...
Astropy には表処理のために \code{Table}\index{table@\code{Table}|textbf} と \code{QTable}\index{qtable@\code{QTable}|textbf} がある。\code{Table} も列の単位をメタデータとして保持できるが、\code{QTable} は単位付きの列を \code{Quantity} として保持し、単位を考慮した演算に使える。
...
天文データでは FITS（Flexible Image Transport System）\index{fits@FITS|textbf} が広く用いられる。HDU（Header/Data Unit）はヘッダとデータをまとめた単位であり、一つの FITS ファイルは複数の HDU を持てる。Astropy なら NumPy 配列やヘッダをそのまま扱える。
...
table.write("events.ecsv", format="ascii.ecsv", overwrite=True)
tbl = Table.read("events.ecsv", format="ascii.ecsv")
print(tbl.colnames)
print(len(tbl))
\end{lstlisting}

\code{Table.read()} の第 1 引数は入力パス、\code{format="ascii.ecsv"} は ECSV として解釈する指定である。上で作った 3 行・3 列の表を保存して読み直すと、\code{['time', 'signal', 'detector']} と \code{3} が表示される。ECSV は、列の単位、データ型、表のメタデータをテキストとともに保存できるため、中間生成物の受け渡しに利用できる。保存時は \code{table.write("events.ecsv", format="ascii.ecsv", overwrite=True)} と書く。\code{overwrite=True} は同名ファイルの置換を許可する指定である。
