...
正規表現の括弧で囲んだ部分は、照合結果から個別に取り出せる。Python では、\code{re.search} や \code{re.fullmatch} の戻り値に対して \code{group()} を呼び出す。
...
正規表現へ条件を詰め込みすぎると、パターンの検証と保守が難しくなる。まずアンカー、文字クラス、量指定子で条件を記述し、それで表現できない場合に括弧や選択を加える。長いパターンには、受理する文字列と拒否する文字列のテストも用意する。
