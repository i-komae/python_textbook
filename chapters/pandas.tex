...
ここで区別したいのは、Parquet がファイル形式の名前であり、\code{pyarrow} はその読書きを担当するライブラリだという点である。\code{to\_parquet()} や \code{read\_parquet()} がライブラリ不足を示すエラーで失敗した場合は、Parquet ファイルの破損ではなく、\code{pyarrow} などの対応ライブラリが環境に入っていない可能性がある。
